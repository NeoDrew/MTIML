\documentclass[20pt,a0paper,landscape,
  ...
  colspace=11mm,subcolspace=6mm]{tikzposter}
...
\definecolor{posterPinkLight}{RGB}{255,241,248}
...
\newcommand{\smallbox}[1]{\begin{center}\fbox{\begin{minipage}{0.88\linewidth}\centering #1\end{minipage}}\end{center}}


%Title
\title{\parbox{\linewidth}{\centering VC Dimension Upper-Bounds Rademacher Complexity\\[-0.25em]
\normalsize A classical route from infinite hypothesis classes to uniform generalisation}}
\author{Jay Parekh, Yeshwanth Patil, Andrew Robertson}
\institute{COMP34312, The University of Manchester}
...
\miniheadline{Interpretation.}
\begin{itemize}
\item Larger VC dimension $d$ means a richer class.
\item Larger sample size $m$ improves generalisation.
\item Finite VC dimension gives uniform convergence.
\end{itemize}
}
...
\begin{tikzpicture}[node distance=0.55cm,scale=0.78,transform shape]
  \node[draw,rounded corners,fill=posterPinkLight,minimum width=3.2cm] (gap) {gap};
  \node[draw,rounded corners,fill=posterPinkLight,minimum width=3.2cm,right=of gap] (rad) {Rademacher};
  \node[draw,rounded corners,fill=posterPinkLight,minimum width=3.2cm,right=of rad] (growth) {growth};
  \node[draw,rounded corners,fill=posterPinkLight,minimum width=3.2cm,right=of growth] (vc) {VC dim};
